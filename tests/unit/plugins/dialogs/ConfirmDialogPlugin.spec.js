...
describe('Confirm Dialog Plugin Testing', () => {
  ...
  describe('ConfirmDialog is visible in the document.', () => {
    it(`The ConfirmDialog is shown on the document.`, done => {
      ...
      setTimeout(() => {
        expect(document.querySelector('.nu-confirm-dialog')).to.not.be.null;
        done();
      }, 500);
    });
    it(`The title inside the header slot of the element equals to ${title}.`, done => {
      wrapper.find('button').trigger('click');
      setTimeout(() => {
        expect(document.querySelector('.nu-confirm-dialog__header').firstChild.textContent.trim()).to.equal(title);
        done();
      }, 500);
    });
    it(`The message inside the header slot of the element equals to ${message}.`, done => {
      wrapper.find('button').trigger('click');
      setTimeout(() => {
        expect(document.querySelector('.nu-confirm-dialog__body').firstChild.textContent.trim()).to.equal(message);
        done();
      }, 500);
    });
  });
